...
class UserPreferencesRepository @Inject constructor(
    ...
    suspend fun saveDailyMixSongIds(songIds: List<String>) {
        dataStore.edit { it[PreferencesKeys.DAILY_MIX_SONG_IDS] = json.encodeToString(songIds) }
    }

    val yourMixSongIdsFlow: Flow<List<String>> =
        pref { decodeJsonPref(it, PreferencesKeys.YOUR_MIX_SONG_IDS, emptyList()) }

    suspend fun saveYourMixSongIds(songIds: List<String>) {
        dataStore.edit { it[PreferencesKeys.YOUR_MIX_SONG_IDS] = json.encodeToString(songIds) }
    }

    val lastDailyMixUpdateFlow: Flow<Long> =
        pref { it[PreferencesKeys.LAST_DAILY_MIX_UPDATE] ?: 0L }

    suspend fun saveLastDailyMixUpdateTimestamp(timestamp: Long) {
        dataStore.edit { it[PreferencesKeys.LAST_DAILY_MIX_UPDATE] = timestamp }
    }

    // ─── Filtering ────────────────────────────────────────────────────────────

    /** Minimum song duration in milliseconds. Default 10 000 ms (10 seconds). */
    val minSongDurationFlow: Flow<Int> =
        pref { (it[PreferencesKeys.MIN_SONG_DURATION] ?: 10_000).coerceIn(0, 120_000) }

    suspend fun setMinSongDuration(durationMs: Int) {
        dataStore.edit { it[PreferencesKeys.MIN_SONG_DURATION] = durationMs.coerceIn(0, 120_000) }
    }

    suspend fun getMinSongDuration(): Int = minSongDurationFlow.first()

    val minTracksPerAlbumFlow: Flow<Int> =
        pref { it[PreferencesKeys.MIN_TRACKS_PER_ALBUM] ?: 1 }

    suspend fun setMinTracksPerAlbum(minTracks: Int) {
        dataStore.edit { it[PreferencesKeys.MIN_TRACKS_PER_ALBUM] = minTracks }
    }

    // ─── ReplayGain ───────────────────────────────────────────────────────────

    val replayGainEnabledFlow: Flow<Boolean> =
        pref { it[PreferencesKeys.REPLAYGAIN_ENABLED] ?: false }

    suspend fun setReplayGainEnabled(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.REPLAYGAIN_ENABLED] = enabled }
    }

    val replayGainUseAlbumGainFlow: Flow<Boolean> =
        pref { it[PreferencesKeys.REPLAYGAIN_USE_ALBUM_GAIN] ?: false }

    suspend fun setReplayGainUseAlbumGain(useAlbumGain: Boolean) {
        dataStore.edit { it[PreferencesKeys.REPLAYGAIN_USE_ALBUM_GAIN] = useAlbumGain }
    }

    // ─── View preferences ─────────────────────────────────────────────────────

    val isGenreGridViewFlow: Flow<Boolean> =
        pref { it[PreferencesKeys.IS_GENRE_GRID_VIEW] ?: true }

    suspend fun setGenreGridView(isGrid: Boolean) {
        dataStore.edit { it[PreferencesKeys.IS_GENRE_GRID_VIEW] = isGrid }
    }

    val isAlbumsListViewFlow: Flow<Boolean> =
        pref { it[PreferencesKeys.IS_ALBUMS_LIST_VIEW] ?: false }

    suspend fun setAlbumsListView(isList: Boolean) {
        dataStore.edit { it[PreferencesKeys.IS_ALBUMS_LIST_VIEW] = isList }
    }
